...
public class DijkstraShortestPath1 {// Member variables of the class  
	...
	public DijkstraShortestPath1(int totalNodes)  
	{  
	...
	if (!settld.contains(vx.vertex))   
	{  
	    edgeDist = vx.cost;  
	    newDist = distance[ux] + edgeDist;  
	  
	    // If the new distance is lesser in the cost  
	    if (newDist < distance[vx.vertex])  
	    {  
	        distance[vx.vertex] = newDist;  
	    }  
	  
	    // Adding the current node to the priority queue pQue  
	    pQue.add(new Node(vx.vertex, distance[vx.vertex]));  
	}  
	}  
	}  
	...
	DijkstraShortestPath1 obj = new DijkstraShortestPath1(totalNodes);  
